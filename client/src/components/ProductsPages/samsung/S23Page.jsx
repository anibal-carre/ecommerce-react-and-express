...
import ProductViewS23 from "../../ProductView/ProductViewS23";
      ...
      <section className="w-full h-[800px] flex justify-center p-8 py-16 border">
        <div className="w-full max-w-[1100px] flex">
          ...
          <div className="w-[700px] px-10 py-5">
            ...
            {!loading ? (
              <Rank rank={data?.Samsung?.[2].ranking} />
            ) : (
              <SkeletonTheme>
                <Skeleton count={1} />
              </SkeletonTheme>
            ...
            {!loading ? (
              <div className="flex items-center gap-2 mt-4">
                ...
                >
                  <div
                    className={`w-[36px] h-[36px] bg-green-600 rounded-full `}
                  ></div>
                </div>
                <div
                  className={`w-[42px] h-[42px] flex justify-center items-center bg-white border rounded-full ${
                    color == "black" ? `border border-[#ff8800]` : ""
                  }`}
                ...
                >
                  <div
                    className={`w-[36px] h-[36px] bg-yellow-50 rounded-full `}
                  ></div>
                </div>
              </div>
            ) : (
                ...
                <div className="w-full mt-2 flex gap-4">
                  ...
                  >
                    ...
                    <span className="text-[12px] font-[400]">
                      {capacity == "128 GB"
                        ? `R$ ${data?.Samsung?.[2].price.toLocaleString()},00`
                        : ""}
                      {capacity == "256 GB"
                        ? `R$ ${(
                            data?.Samsung?.[2].price * 1.3
                          ).toLocaleString()},00`
                        : ""}
                    </span>
                  </button>
            ...
            {!loading ? (
              <span className="text-[32px] font-[700] text-zinc-700">
                ...
                {condition == "excelente" && capacity == "128 GB"
                  ...
                  : ""}
                {condition == "excelente" && capacity == "256 GB"
                  ? `R$ ${(data?.Samsung?.[2].price * 1.3).toLocaleString()},00`
                  : ""}

                {/* CONDITION = MUITO BOM */}

                {condition == "muito bom" && capacity == "128 GB"
                  ? `R$ ${(data?.Samsung?.[2].price - 300).toLocaleString()},00`
                  : ""}
                {condition == "muito bom" && capacity == "256 GB"
                  ? `R$ ${(
                      (data?.Samsung?.[2].price - 300) *
                ...
                {condition == "bom" && capacity == "128 GB"
                  ? `R$ ${(data?.Samsung?.[2].price - 500).toLocaleString()},00`
                  : ""}
                {condition == "bom" && capacity == "256 GB"
                  ? `R$ ${(
                      (data?.Samsung?.[2].price - 500) *
            ...
            {!loading ? (
              <span className="mt-2 font-[500]">
                ...
                <span className="text-zinc-400 font-[600] text-[17px]">
                  ...
                  {condition == "excelente" && capacity == "128 GB"
                    ? `R$ ${(
                        data?.Samsung?.[2].price * 1.1
                      ).toLocaleString()},00`
                    : ""}
                  {condition == "excelente" && capacity == "256 GB"
                    ? `R$ ${(
                        data?.Samsung?.[2].price *
                        1.3 *
                        1.08
                      ).toLocaleString()},00`
                    : ""}
            ...
            {!loading ? (
              <span className="font-[400] text-[14px]">
                ...
                <span className="font-[600]">
                  ...
                  {condition == "excelente" && capacity == "128 GB"
                    ? `R$ ${((data?.Samsung?.[2].price * 1.1) / 12)
                        .toFixed(2)
                        .toLocaleString()}`
                    : ""}
                  {condition == "excelente" && capacity == "256 GB"
                    ? `R$ ${((data?.Samsung?.[2].price * 1.3 * 1.08) / 12)
                        .toFixed(2)
                        .toLocaleString()}`
                    : ""}
                  {/* CONDITION = MUITO BOM */}
                  {condition == "muito bom" && capacity == "128 GB"
                    ? `R$ ${(((data?.Samsung?.[2].price - 300) * 1.1) / 12)
                        .toFixed(2)
                        .toLocaleString()}`
                    : ""}
                  {condition == "muito bom" && capacity == "256 GB"
                    ? `R$ ${(
                        ((data?.Samsung?.[2].price - 300) * 1.3 * 1.08) /
                        12
                      )
                        .toFixed(2)
                        .toLocaleString()}`
                  ...
                  {/* CONDITION = BOM */}
                  {condition == "bom" && capacity == "128 GB"
                    ? `R$ ${(((data?.Samsung?.[2].price - 500) * 1.1) / 12)
                        .toFixed(2)
                        .toLocaleString()}`
                    : ""}
                  {condition == "bom" && capacity == "256 GB"
                    ? `R$ ${(
                        ((data?.Samsung?.[2].price - 500) * 1.2 * 1.01) /
                        12
                      )
                        .toFixed(2)
                        .toLocaleString()}`
